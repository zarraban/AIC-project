...
@router.get("/vashchenko/product-sales-stats/{product_id}")
async def get_product_sales_stats(product_id: int, db: psycopg.AsyncConnection = Depends(get_db)):
    try:
        async with db.cursor() as cursor:
            await cursor.execute(
                ...
                SELECT
                    ...
                    COUNT(s.receipt_number) AS receipt_count,
                    SUM(s.product_number) AS items_sold,
                ...
                GROUP BY sp.upc, sp.promotional_product
...
@router.get("/vashchenko/categories-bought-by-all")
async def get_categories_bought_by_all(db: psycopg.AsyncConnection = Depends(get_db)):
    try:
        async with db.cursor() as cursor:
            await cursor.execute(
                """
                ...
                WHERE NOT EXISTS (
                    ...
                    WHERE NOT EXISTS (
                        ...
                        WHERE p.category_number = cat.category_number
                          AND r.card_number = card.card_number
                    )
                );
                """
            )
